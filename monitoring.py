    ...
    try:
        ...
        while True:
            ...
            for account in accounts:
                for ticker in tickers:
                    if account['market'] == ticker['market']:
                        ...
                        # 개별 변동률 계산
                        # -------------------------------------------------------------
                        chg_rate = round(((Decimal(str(ticker['trade_price'])) - Decimal(
                            str(account['avg_buy_price']))) / Decimal(str(account['avg_buy_price']))) * Decimal(
                            str(100)), 2)

                        # 개별 종목 10% 이상 상승 시 메세지 발송(1시간 간격)
                        if Decimal(str(chg_rate)) >= Decimal(str(10)):
                            logging.info(
                                "PCNT-UP 조건 만족![" + str(account['market']) + "]")
                        ...
                        if Decimal(str(chg_rate)) <= Decimal(str(-10)):
                            logging.info(
                                "PCNT-DOWN 조건 만족![" + str(account['market']) + "]")
                            logging.info("변동률: [" + str(chg_rate) + "% ]")

                            # 알림 Key 조립
                            msg_key = {'TYPE': 'PCNT-DOWN',
                                       'ITEM': account['market']}
                        ...
                        # -------------------------------------------------------------
                        # 현재가격 합계(평가금액)
                        cur_price_sum = Decimal(str(cur_price_sum)) + (
                            Decimal(str(ticker['trade_price'])) * Decimal(str(account['balance'])))

                        # 매수금액 합계
                        avg_buy_price_sum = Decimal(str(avg_buy_price_sum)) + (
                            Decimal(str(account['avg_buy_price'])) * Decimal(str(account['balance'])))

            # 전체 수익률
            overall_amt = cur_price_sum - avg_buy_price_sum
            overall_revenue = round(
                ((cur_price_sum - avg_buy_price_sum) / avg_buy_price_sum) * Decimal(str(100)), 2)
            ...
            if Decimal(str(overall_revenue)) >= Decimal(str(5)):
                ...
                message = '\n\n[전체 자산 상승안내]'
                message = message + '\n\n- 전체 매수금액: ' + \
                    '{:0,.0f}'.format(round(avg_buy_price_sum, 0)) + '원'
                message = message + '\n- 전체 평가금액: ' + \
                    '{:0,.0f}'.format(round(cur_price_sum, 0)) + '원'
                message = message + '\n- 전체 변동금액: ' + \
                    '{:0,.0f}'.format(round(overall_amt, 0)) + '원'
                message = message + '\n- 전체 수익률:  ' + \
                    str(overall_revenue) + '%'
