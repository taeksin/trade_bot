...
# headers   :  웹사이트를 호출할 때 간혹 사용자가 아닌 봇이 접근하는 것을 차단하는 
#             사이트들이 있어서 예방 차원에서 넣어둔 부분입니다. 위의 내용은 Chrome 브라우저에서 
#             접근한 듯한 User-Agent를 가지고 웹사이트에 접속하도록 합니다.
# page_url  :  업비트 공지사항 리스트를 받아오는 URL입니다.
# query_url :  텔레그램 메세지로 받은 URL을 클릭하여 개별 공지를 쉽게 확인할 수 있도록 개별 
#             공지사항이 열리는 URL입니다. 뒤에 게시물 ID를 붙여주면 공지를 바로 확인할 
#             수 있습니다.
# -----------------------------------------------------------------------------
# - Name : mon_notice
# - Desc : 업비트 공지사항 크롤링
    ...
    try:
        ...
        while True:
            ...
            for notice in upbit_notice['data']['list']:
                ...
                print(len(list(filter(lambda x: x['title'] == title, prev_notice_list))))
                print("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ")
                # 기존 공지내역이 있으면 비교하기
