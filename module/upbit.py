...
def sellcoin_tg(target_item, sell_price):
    '''손으로 매매를 하는 경우 지정가 매도를 할 때 1000원에 팔아야 하는데 0을 하나 빼먹고 100을
    입력하는 경우와 같이 실수로 시장 가격보다 낮게 매도를 거는 경우가 발생할 수도 있는데요. 이렇게
    지정가 매도를 할 때 시장가 보다 낮게 설정하면 어떻게 될까요? 이런 경우 100원에 매도가 되는 것이
    아니라 시장가격에 매도가 되기 때문에 크게 걱정하실 필요는 없습니다. 예를들어 현재 150원에 매수세가
    있고 155원에 매도세가 있다고 가정할 때 실수로 지정가 매도를 130원에 걸게 되면 130원에 매도가 되는 것이
    아니라 현재 시장 매수가격인 150원에 매도 처리가 됩니다. 프로그램을 통해 매매를 하는 경우에는 손으로 매도가를
    입력하지 않기 때문에 실수를 줄일 수 있으며 또는 위와 같은 로직을 활용하여 급등하는 코인에서는 지정가 매도에서
    시장가 매도의 기능을 일부러 활용할 수도 있습니다.'''
    try:
...
def get_targetprice(cal_type, st_price, chg_val):
    ''' ① 100만원 금액에 해당하는 호가를 출력 get_hoga('1000000') 을 호출한 결과 100만원의 호가인 500원이 리턴 되었습니다.
        ② 100만원 금액 기준 10개 위의 호가를 계산해 지정가를 출력 get_targetprice('H', '1000000', '10')로 호출하여
           100만원의 호가 단위인 500원을 계산하고 10개 위의 호가인 5000원을 더해 1,005,000원이 리턴 되었습니다.
        ③ 100만원 금액 기준 10% 위의 호가를 계산해 지정가를 출력 get_targetprice('R', '1000000', '10')로 호출하여
           100만원의 10% 상승 가격에 대한 호가인 1,100,000원을 리턴하였습니다.'''
    try:
        # 계산된 가격
        rtn_price = st_price
...
def get_accounts(except_yn, market_code):
    '''① except_yn : (Y 또는 N) Y로 변수를 넘기면 KRW 잔액 및 소액(5,000원)
        미만 보유 종목은 제외하고 조회 할 수 있습니다. 그 외에는 KRW 및 소액 보유 종목을 모두 리턴하게 됩니다.
       ② market_code : KRW, BTC, USDT 등 마켓코드 잔고 정보에는 마켓 코드가 붙어있지 않습니다. 매도를 하기
        위해서는 마켓 코드가 붙은 종목 코드(KRW-DOGE)가 필요하기 때문에 매도를 하려는 마켓 코드를 넘기면 코드를 붙여서 리턴하게 됩니다.'''
    try:
...
#     (candle_datas) : 캔들 정보
#       ㄴ target_item : 대상 종목
#       ㄴ tick_kind : 캔들 종류 (1, 3, 5, 10, 15, 30, 60, 240 - 분, D-일, W-주, M-월)
#       ㄴ inq_range : 조회 범위
# - Output
#   1) RSI 값
# -----------------------------------------------------------------------------
